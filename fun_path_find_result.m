function [X, f] = fun_path_find_result(connectnode, first_node, exit, temperature, reliability, node_location, figurenum)
...
for i = 1:node_num
    ...
    node_temperature = temperature(connect_select);
    node_r = reliability(connect_select);
    
    for j = 1:list_length
        t((i-1)*node_num+connect_select(j)) = node_temperature(j);
...
for i = 1:node_num
    ...
    text( x(i)+0.2, y(i)-0.4, ['\color{red}', num2str(temperature(i))]);
