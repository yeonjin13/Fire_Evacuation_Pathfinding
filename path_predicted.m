...
centered_temp = csvread('centered_temp_per_node.csv');
time = centered_temp(2:end, 1);

L = 55;
w = 19;

tempsum = zeros(1, size(connectnode,1));
for t = L:L+19
...
reliability = ones(size(connectnode, 1), 1);
for node = 1:47
    diffsum = 0;
    diff = 0;
    for t = L:L+19
        ...
        diffsum = diffsum+abs(centered_temp_of_pred_clus - centered_temp_of_real_clus);
    end
    diff = diffsum / 20;
    reliability(node, 1) = diff;
end
