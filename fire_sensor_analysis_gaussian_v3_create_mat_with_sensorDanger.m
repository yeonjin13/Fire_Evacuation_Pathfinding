...
for picked_time_idx= 491:25:991
    dynamics = csvread('NEW_data.csv');
    ...
    num_nodes = size(fire_temp);
    current_fire_temp = fire_temp;
    
    for index = 2:num_nodes(1,2)
        if(fire_temp(1,index) >= 150)
            fire_temp(1,index) = max(fire_temp);
        end
        if(sensor_danger(1,index) == 1)
            fire_temp(1,index) = max(fire_temp);
        end
    end
    ...
    orient portrait;
    saveas(figure(picked_time_idx),['simu_',num2str(picked_time),'.png']);
